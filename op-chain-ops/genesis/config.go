...
type DeployConfig struct {
	...
	// generally set to 1 week for Optimism, but could in theory be set to 0 for a safe ZK Rollup.
	...
	// L2GenesisL2CancunTimeOffset is the number of seconds after genesis block that L2 cancun hard fork activates.
	// Set it to 0 to activate at genesis. Nil to disable Ecotone.
	L2GenesisL2CancunTimeOffset *hexutil.Uint64 `json:"l2GenesisL2CancunTimeOffset,omitempty"`
	...
	// L2GenesisHyraxTimeOffset is the number of seconds after genesis block that Hyrax hard fork activates.
	// Set it to 0 to activate at genesis. Nil to disable Hyrax.
	L2GenesisHyraxTimeOffset *hexutil.Uint64 `json:"l2GenesisHyraxTimeOffset,omitempty"`
	...
	// VerifierProxy represents the address of the VerifierProxy on L1 and is used as part of
	// output verification from the L2OutputOracle
	...
	// Scroll Circuit Checker Limitations [optional]
	MaxTxPerBlock             *int `json:"maxTxPerBlock,omitempty"`
	MaxTxPayloadBytesPerBlock *int `json:"maxTxPayloadBytesPerBlock,omitempty"`

	// convenient for testing in local deployment
	ZKTrieSwitchBlock *big.Int `json:"zkTrieSwitchBlock,omitempty"`
}
...
func (d *DeployConfig) Check() error {
	...
	if d.MaxTxPerBlock != nil && *d.MaxTxPerBlock <= 0 {
		return fmt.Errorf("%w: L2 genesis max tx per block must be > 0", ErrInvalidDeployConfig)
	}
	if d.MaxTxPayloadBytesPerBlock != nil && *d.MaxTxPayloadBytesPerBlock <= 0 {
		return fmt.Errorf("%w: L2 genesis max tx payload bytes per block must be > 0", ErrInvalidDeployConfig)
	}
	...
	if err := checkFork(d.L2GenesisEcotoneTimeOffset, d.L2GenesisHyraxTimeOffset, "ecotone", "hyrax"); err != nil {
		return err
	}
	return nil
}
...
func (d *DeployConfig) CheckAddresses() error {
	...
	if d.VerifierProxy == (common.Address{}) {
		return fmt.Errorf("%w: VerifierProxy cannot be address(0)", ErrInvalidDeployConfig)
	}
	return nil
}
...
func (d *DeployConfig) SetDeployments(deployments *L1Deployments) {
	...
	d.VerifierProxy = deployments.VerifierProxy
}
...
func (d *DeployConfig) L2CancunTime(genesisTime uint64) *uint64 {
	if d.L2GenesisL2CancunTimeOffset == nil {
		return nil
	}
	v := uint64(0)
	if offset := *d.L2GenesisL2CancunTimeOffset; offset > 0 {
		v = genesisTime + uint64(offset)
	}
	return &v
}
...
func (d *DeployConfig) HyraxTime(genesisTime uint64) *uint64 {
	if d.L2GenesisHyraxTimeOffset == nil {
		return nil
	}
	v := uint64(0)
	if offset := *d.L2GenesisHyraxTimeOffset; offset > 0 {
		v = genesisTime + uint64(offset)
	}
	return &v
}
...
func (d *DeployConfig) RollupConfig(l1StartBlock *types.Block, l2GenesisBlockHash common.Hash, l2GenesisBlockNumber uint64) (*rollup.Config, error) {
	...
	return &rollup.Config{
		...
		L2CancunTime:           d.L2CancunTime(l1StartBlock.Time()),
		FjordTime:              d.FjordTime(l1StartBlock.Time()),
		InteropTime:            d.InteropTime(l1StartBlock.Time()),
		HyraxTime:              d.HyraxTime(l1StartBlock.Time()),
	}, nil
}
...
func (d *ForgeAllocs) UnmarshalJSON(b []byte) error {
	...
	d.Accounts = make(core.GenesisAlloc, len(allocs))
