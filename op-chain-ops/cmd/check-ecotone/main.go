...
func checkBeaconBlockRoot(ctx context.Context, env *actionEnv) error {
	...
	if result.StorageHash == types.EmptyLegacyTrieRootHash || result.StorageHash == types.EmptyZkTrieRootHash {
	...
	headRef, err := derive.PayloadToBlockRef(rollupCfg, payload.ExecutionPayload, nil)
	if err != nil {
