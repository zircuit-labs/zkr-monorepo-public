...
import (
	...
	"github.com/zircuit-labs/zkr-monorepo-public/op-node/node"
	p2pcli "github.com/zircuit-labs/zkr-monorepo-public/op-node/p2p/cli"
...
func NewConfig(ctx *cli.Context, log log.Logger) (*node.Config, error) {
	...
	// set NATS config if enabled
	if ctx.Bool(flags.NATSEnabledFlag.Name) {
		log.Info("NATS enabled")
		nats_addr := os.Getenv("CFG_NATS_ADDRESS")
		// nats_addr is of form: `nats://<host>:4222` but
		// the leafnode URL should be of form: `nats-leaf://<host>` (no port)
		u, err := url.Parse(nats_addr)
		if err != nil {
			return nil, fmt.Errorf("failed to parse nats address: %w", err)
		}
		u.Scheme = "nats-leaf"
		...
		value, enableLogging := os.LookupEnv("CFG_NATS_ENABLELOGGING")
		if enableLogging && strings.ToLower(value) == "false" {
			enableLogging = false
		}

		cfg.NatsConfig = &node.NATSConfig{
			LeafNodeURL:           u.String(),
			StoreDir:              ctx.String(flags.NATSStoreDirFlag.Name),
			RemoteCredentialsPath: os.Getenv("CFG_NATS_CREDENTIALSPATH"),
...
func NewL1EndpointConfig(ctx *cli.Context) *node.L1EndpointConfig {
	return &node.L1EndpointConfig{
		...
		L1RPCKind:        l1.RPCProviderKind(strings.ToLower(ctx.String(flags.L1RPCProviderKind.Name))),
