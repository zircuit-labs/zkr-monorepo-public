...
func TestAttributesHandler(t *testing.T) {
	...
	aL1Info := &testutils.MockBlockInfo{
		...
		InfoReceiptRoot: types.EmptyReceiptsHash,
	...
	a1L1Info, err := derive.L1InfoDepositBytes(cfg, cfg.Genesis.SystemConfig, 1, aL1Info, nil, refA0.Time+cfg.BlockTime)
	...
	refA1, err := derive.PayloadToBlockRef(cfg, payloadA1.ExecutionPayload, payloadA1.L1Info)
	require.NoError(t, err)
	...
	refA1Alt, err := derive.PayloadToBlockRef(cfg, payloadA1Alt.ExecutionPayload, payloadA1.L1Info)
	require.NoError(t, err)
