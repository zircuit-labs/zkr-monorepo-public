...
type Config struct {
	...
	// L2CancunTime sets the activation time of cancun on L2.
	// Active if L2CancunTime != nil && L2 block timestamp >= *L2CancunTime, inactive otherwise.
	L2CancunTime *uint64 `json:"l2cancun_time,omitempty"`
	...
	// HyraxTime sets the activation time of the Hyrax network upgrade.
	// Active if HyraxTime != nil && L2 block timestamp >= *HyraxTime, inactive otherwise.
	HyraxTime *uint64 `json:"hyrax_time,omitempty"`
...
func (cfg *Config) Check() error {
	...
	if err := checkFork(cfg.EcotoneTime, cfg.L2CancunTime, Ecotone, L2Cancun); err != nil {
		return err
	}
...
func (c *Config) L1Signer() l1types.Signer {
	return l1types.NewCancunSigner(c.L1ChainID)
}
...
// IsHyrax returns true if the Hyrax hardfork is active at or past the given timestamp.
func (c *Config) IsHyrax(timestamp uint64) bool {
	return c.HyraxTime != nil && timestamp >= *c.HyraxTime
}
...
// IsL2Cancun returns true if the cancun hardfork on L2 is active at or past the given timestamp.
func (c *Config) IsL2Cancun(timestamp uint64) bool {
	return c.L2CancunTime != nil && timestamp >= *c.L2CancunTime
}
...
func (c *Config) Description(l2Chains map[string]string) string {
	...
	banner += fmt.Sprintf("  - L2Cancun: %s\n", fmtForkTimeOrUnset(c.L2CancunTime))
	banner += fmt.Sprintf("  - Fjord: %s\n", fmtForkTimeOrUnset(c.FjordTime))
	banner += fmt.Sprintf("  - Interop: %s\n", fmtForkTimeOrUnset(c.InteropTime))
	banner += fmt.Sprintf("  - Hyrax: %s\n", fmtForkTimeOrUnset(c.HyraxTime))
	return banner
}
...
func (c *Config) LogDescription(log log.Logger, l2Chains map[string]string) {
	...
	log.Info("Rollup Config", "l2_chain_id", c.L2ChainID, "l2_network", networkL2, "l1_chain_id", c.L1ChainID,
		...
		"l2cancun_time", fmtForkTimeOrUnset(c.L2CancunTime),
		"fjord_time", fmtForkTimeOrUnset(c.FjordTime),
		"interop_time", fmtForkTimeOrUnset(c.InteropTime),
		"hyrax_time", fmtForkTimeOrUnset(c.HyraxTime),
	)
}
