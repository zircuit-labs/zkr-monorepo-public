...
func (m *FakeAttributesBuilder) PreparePayloadAttributes(ctx context.Context,
	l2Parent eth.L2BlockRef, epoch eth.BlockID, exclusions *types.Bitmap,
...
func createSequencer(log log.Logger) (*Sequencer, *sequencerTestDeps) {
	...
	seq.toBlockRef = func(rollupCfg *rollup.Config, payload *eth.ExecutionPayload, l1Info *types.L1Info) (eth.L2BlockRef, error) {
...
func TestSetDepositExclusionBitmap(t *testing.T) {
	rng := rand.New(rand.NewSource(1234))

	for testRun := 0; testRun < 10000; testRun++ {
		// generate 10 random transactions
		txCount := 10
		txsEncoded := make([]hexutil.Bytes, 0, txCount)
		txHashes := make([]common.Hash, 0, txCount)
		for i := 0; i < txCount; i++ {
			tx := testutils.RandomDepositTx(rng)
		...
		expectedBitmap := *derive.EmptyBitmap(0)
		rejectedTransactions := make([]common.Hash, 0)
		// randomly exclude up to 4 transactions (does not matter if duplicate)
		for i := 0; i < 4; i++ {
			idx := rng.Intn(txCount)
			rejectedTransactions = append(rejectedTransactions, txHashes[idx])
			expectedBitmap.Set(idx)
		}

		// build the bitmap and check that it matches
		depositExclusions := *derive.EmptyBitmap(0)
		require.NoError(t, SetDepositExclusionBitmap(depositExclusions, txsEncoded, rejectedTransactions))
		t.Log("1st iter")
		t.Logf("Expected bitmap:    %v\n", expectedBitmap)
		t.Logf("Deposit exclusions: %v\n", depositExclusions)
		require.Equal(t, expectedBitmap, depositExclusions)

		// second iteration, remove the excluded transactions from the input and reuse the bitmap
		for i := txCount - 1; i >= 0; i-- {
			if expectedBitmap.Test(i) {
				txsEncoded = append(txsEncoded[:i], txsEncoded[i+1:]...)
			}
		}

		// randomly exclude up to 4 more (duplicates just means some variation in the outcome)
		rejectedTransactions = make([]common.Hash, 0)
		for i := 0; i < 4; i++ {
			idx := rng.Intn(txCount)
			rejectedTransactions = append(rejectedTransactions, txHashes[idx])
		...
		t.Logf("Expected bitmap:    %v\n", expectedBitmap)
		t.Logf("Deposit exclusions: %v\n", depositExclusions)
		require.Equal(t, depositExclusions, expectedBitmap)
	}
}
