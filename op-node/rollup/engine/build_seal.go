...
type PayloadSealRejectedTxsEvent struct {
	RejectedPayloadM *eth.RejectedPayloadMeta
}

func (ev PayloadSealRejectedTxsEvent) String() string {
	return "payload-seal-rejected-txs"
}
...
func (eq *EngDeriver) onBuildSeal(ev BuildSealEvent) {
	...
	if envelope.ExecutionErr != nil {
		eq.log.Info("Received a failed payload", "Reason", envelope.ExecutionErr.Err, "reject", *envelope.RejectedPayloadM)
		if envelope.RejectedPayloadM != nil {
			// raise event to redo the payload
			eq.emitter.Emit(PayloadSealRejectedTxsEvent{
				RejectedPayloadM: envelope.RejectedPayloadM,
			})
			// returning and will reattempt right away
			return
		}
		// Also marking as a temporary failure
		eq.emitter.Emit(PayloadSealExpiredErrorEvent{
			Info:         ev.Info,
			Err:          fmt.Errorf("failed to seal execution payload (ID: %s): %w", ev.Info.ID, err),
			IsLastInSpan: ev.IsLastInSpan,
	...
	ref, err := derive.PayloadToBlockRef(eq.cfg, envelope.ExecutionPayload, envelope.L1Info)
	if err != nil {
		eq.emitter.Emit(PayloadSealInvalidEvent{
			Info:         ev.Info,
