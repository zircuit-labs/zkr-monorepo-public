...
func TestAttributesQueue(t *testing.T) {
	...
	l1Fetcher.EXPECT().
		InfoByHash(gomock.Any(), l1Info.InfoHash).
		Return(l1Info, nil).
		Times(1)
	...
	l1InfoTx, err := L1InfoDepositBytes(&rollupCfg, expectedL1Cfg, safeHead.SequenceNumber+1, l1Info, nil, 0)
	require.NoError(t, err)
	l1BlockInfo, err := L1BlockInfoFromParts(&rollupCfg, expectedL1Cfg, safeHead.SequenceNumber+1, l1Info, safeHead.Time+cfg.BlockTime, nil)
	require.NoError(t, err)
