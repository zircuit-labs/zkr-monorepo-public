...
type L1ReceiptsFetcher interface {
	...
	FetchReceipts(ctx context.Context, blockHash common.Hash) (eth.BlockInfo, l1types.Receipts, error)
}
...
func (ba *FetchingAttributesBuilder) PreparePayloadAttributes(ctx context.Context, l2Parent eth.L2BlockRef, epoch eth.BlockID, exclusions *types.Bitmap) (attrs *eth.PayloadAttributes, err error) {
	...
	l1OriginChanged := l2Parent.L1Origin.Number != epoch.Number
	...
	if l1OriginChanged {
		...
		deposits, err := DeriveDeposits(receipts, ba.rollupCfg.DepositContractAddress, exclusions)
		if err != nil {
	...
	// Post Hyrax activation, we only include a system transaction when the L1 origin changes
	if !ba.rollupCfg.IsHyrax(nextL2Time) || l1OriginChanged {
		l1InfoTx, err := L1InfoDepositBytes(ba.rollupCfg, sysConfig, seqNumber, l1Info, exclusions, nextL2Time)
		if err != nil {
	...
	if ba.rollupCfg.IsL2Cancun(nextL2Time) {
	...
	l1BlockInfo, err := L1BlockInfoFromParts(ba.rollupCfg, sysConfig, seqNumber, l1Info, nextL2Time, exclusions)
	if err != nil {
