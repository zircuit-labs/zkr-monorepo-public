...
//
// [Frame Format]: https://github.com/zircuit-labs-optimism/specs/blob/main/specs/protocol/derivation.md#frame-format
...
func (co *SingularChannelOut) AddBlock(rollupCfg *rollup.Config, block *types.Block, l1Info *L1BlockInfo) error {
	if co.closed {
		return ErrChannelOutAlreadyClosed
	}

	batch, err := BlockToSingularBatch(rollupCfg, block, l1Info)
	if err != nil {
		return err
	}
...
func BlockToSingularBatch(rollupCfg *rollup.Config, block *types.Block, l1Info *L1BlockInfo) (*SingularBatch, error) {
	...
	var depositExclusions hexutil.Bytes
	if l1Info.DepositExclusions != nil {
		depositExclusions = l1Info.DepositExclusions.MustBytes()
	}
	...
	return &SingularBatch{
		...
		DepositExclusions: depositExclusions,
	}, nil
}
