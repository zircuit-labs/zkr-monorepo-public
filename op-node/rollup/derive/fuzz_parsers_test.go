...
import (
	...
	l1rawdb "github.com/ethereum/go-ethereum/core/rawdb"
	l1state "github.com/ethereum/go-ethereum/core/state"
	l1tracing "github.com/ethereum/go-ethereum/core/tracing"
	l1runtime "github.com/ethereum/go-ethereum/core/vm/runtime"
	l1crypto "github.com/ethereum/go-ethereum/crypto"
	l1triedb "github.com/ethereum/go-ethereum/triedb"
	...
	"github.com/zircuit-labs/zkr-monorepo-public/op-bindings/bindings"
...
var (
	pk, _   = l1crypto.GenerateKey()
	opts, _ = bind.NewKeyedTransactorWithChainID(pk, common.Big1)
	from    = l1crypto.PubkeyToAddress(pk.PublicKey)
)
...
func FuzzL1InfoBedrockRoundTrip(f *testing.F) {
	f.Fuzz(func(t *testing.T, number, time uint64, baseFee, hash []byte, seqNumber uint64) {
		in := NewL1BlockInfo(&types.L1Info{
			Number:         number,
			Time:           time,
		...
		out := NewL1BlockInfo(&types.L1Info{})
...
func FuzzL1InfoBedrockAgainstContract(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, number, time uint64, baseFee, hash []byte, seqNumber uint64, batcherHash []byte, l1FeeOverhead []byte, l1FeeScalar []byte) {
		expected := NewL1BlockInfo(&types.L1Info{
			Number:         number,
			Time:           time,
			BaseFee:        BytesToBigInt(baseFee),
			BlockHash:      common.BytesToHash(hash),
...
func EncodeDepositOpaqueDataV0(t *testing.T, mint *big.Int, value *big.Int, gasLimit uint64, isCreation bool, data []byte) []byte {
	...
	// See https://github.com/zircuit-labs/l2-geth-public/issues/22257
...
func FuzzUnmarshallLogEvent(f *testing.F) {
	...
	state, err := l1state.New(l1common.Hash(types.EmptyLegacyTrieRootHash), l1state.NewDatabase(l1triedb.NewDatabase(l1rawdb.NewMemoryDatabase(), nil), nil))
	require.NoError(f, err)
	state.SetBalance(from, uint256.MustFromBig(BytesToBigInt([]byte{0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff})), l1tracing.BalanceChangeUnspecified)
	_, addr, _, err := l1runtime.Create(common.FromHex(bindings.OptimismPortalMetaData.Bin), &l1runtime.Config{
		Origin:   from,
		State:    state,
		GasLimit: 20_000_000,
	})
	...
	portalContract, err := bindings.NewOptimismPortal(common.Address(addr), nil)
	require.NoError(f, err)
	...
	f.Fuzz(func(t *testing.T, _to, _mint, _value, data []byte, l2GasLimit uint64, isCreation bool) {
		...
		_, _, err = l1runtime.Call(addr, tx.Data(), &cfg)
		logs := state.Logs()
		...
		l2Log := l1eth.ConvertL1LogToL2(*logs[0])
		depositEvent, err := portalContract.ParseTransactionDeposited(l2Log)
		if err != nil {
