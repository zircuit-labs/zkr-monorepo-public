...
import (
	...
	"github.com/zircuit-labs/zkr-monorepo-public/op-service/testutils/fuzzerutils"
)

// fuzzReceipts is similar to makeReceipts except it uses the fuzzer to populate DepositTx fields.
func fuzzReceipts(typeProvider *fuzz.Fuzzer, blockHash l1common.Hash, depositContractAddr common.Address) (receipts []*l1types.Receipt, expectedDeposits []*types.DepositTx) {
	...
	f.Fuzz(func(t *testing.T, fuzzedData []byte) {
		...
		for _, receipt := range receipts {
			...
			for _, log := range receipt.Logs {
				if log.Address == l1common.Address(MockDepositContractAddr) && len(log.Topics) >= 4 && log.Topics[0] == l1common.Hash(DepositEventABIHash) {
