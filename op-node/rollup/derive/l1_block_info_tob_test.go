...
func FuzzParseL1InfoDepositTxDataValid(f *testing.F) {
	f.Fuzz(func(t *testing.T, fuzzedData []byte) {
		...
		depTx, err := L1InfoDeposit(&rollupCfg, sysCfg, seqNr, &l1Info, 0, nil)
		...
		require.EqualValues(t, res.L1FeeOverhead, sysCfg.Overhead)
		require.EqualValues(t, res.L1FeeScalar, sysCfg.Scalar)
	})
}
...
func FuzzDecodeDepositTxDataToL1Info(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, fuzzedData []byte) {
		...
		depTx, err := L1InfoDeposit(&rollupCfg, sysCfg, res.SequenceNumber, &l1Info, 0, nil)
		require.NoError(t, err, "error creating deposit tx from L1 info")
