...
//
// SingularBatchType := 0
// singularBatch := SingularBatchType ++ RLP([parent_hash, epoch_number, epoch_hash, timestamp, transaction_list, deposit_exclusions])
...
type SingularBatch struct {
	...
	DepositExclusions hexutil.Bytes `rlp:"optional"`
}
...
func (b *SingularBatch) LogContext(log log.Logger) log.Logger {
	return log.New(
		...
		"deposit_exclusions", MustBitmap(b.DepositExclusions),
	)
}
