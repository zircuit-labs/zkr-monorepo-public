...
func TestParseL1InfoDepositTxData(t *testing.T) {
	...
	for i, testCase := range cases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			depTx, err := L1InfoDeposit(&rollupCfg, l1Cfg, seqNr, info, 0, nil)
			...
			assert.EqualValues(t, res.L1FeeOverhead, l1Cfg.Overhead)
			assert.EqualValues(t, res.L1FeeScalar, l1Cfg.Scalar)
		})
	}
	...
	t.Run("invalid selector", func(t *testing.T) {
		...
		depTx, err := L1InfoDeposit(&rollupCfg, randomL1Cfg(rng, info), randomSeqNr(rng), info, 0, nil)
		require.NoError(t, err)
	...
	t.Run("ecotone", func(t *testing.T) {
		...
		depTx, err := L1InfoDeposit(&rollupCfg, randomL1Cfg(rng, info), randomSeqNr(rng), info, 1, nil)
		require.NoError(t, err)
		require.False(t, depTx.IsSystemTransaction)
		require.Equal(t, depTx.Gas, uint64(RegolithSystemTxGas))
	...
	t.Run("first-block ecotone", func(t *testing.T) {
		...
		depTx, err := L1InfoDeposit(&rollupCfg, randomL1Cfg(rng, info), randomSeqNr(rng), info, 2, nil)
		require.NoError(t, err)
		require.False(t, depTx.IsSystemTransaction)
		require.Equal(t, depTx.Gas, uint64(RegolithSystemTxGas))
...
func TestL1BlockInfoMarshalBinaryBedrock(t *testing.T) {
	testCases := []struct {
		Name string
		Info *L1BlockInfo
	}{
		{
			Name: "without exclusions",
			Info: NewL1BlockInfo(&types.L1Info{
				Number:            123,
				Time:              1715828406,
				BaseFee:           big.NewInt(11223344),
				BlockHash:         common.HexToHash("0x36cb48eea9f188c92cbe4507062cde28f9da66b690ea76f1dc7b725896878313"),
				SequenceNumber:    45678,
				BatcherAddr:       common.HexToAddress("0x492c193f068B11ee52a017577805188e45aF517a"),
		...
		},
		{
			...
			Info: NewL1BlockInfo(&types.L1Info{
				...
				L1FeeOverhead:     [32]byte{},
				L1FeeScalar:       [32]byte{},
				DepositExclusions: RandomBitmapNonEmpty(5),
			}),
		},
	}
	...
	for _, testCase := range testCases {
		t.Run(testCase.Name, func(t *testing.T) {
			bin, err := testCase.Info.marshalBinaryBedrock()
			require.NoError(t, err)

			info := NewL1BlockInfo(&types.L1Info{})
			err = info.unmarshalBinaryBedrock(bin)
			require.NoError(t, err)
...
func TestL1BlockInfoMarshalBinaryEcotone(t *testing.T) {
	testCases := []struct {
		Name string
		Info *L1BlockInfo
	}{
		{
			...
			Info: NewL1BlockInfo(&types.L1Info{
				...
				SequenceNumber:    45678,
				BatcherAddr:       common.HexToAddress("0x492c193f068B11ee52a017577805188e45aF517a"),
				BlobBaseFee:       big.NewInt(9988),
				BaseFeeScalar:     789,
				BlobBaseFeeScalar: 234,
				DepositExclusions: nil,
			}),
		},
		{
	...
	for _, testCase := range testCases {
		t.Run(testCase.Name, func(t *testing.T) {
			bin, err := testCase.Info.marshalBinaryEcotone()
			require.NoError(t, err)

			info := NewL1BlockInfo(&types.L1Info{})
			err = info.unmarshalBinaryEcotone(bin)
			require.NoError(t, err)
