...
func TestL1TraversalAdvance(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			startBlock:  a,
			nextBlock:   eth.L1BlockRef{},
			fetcherErr:  l1ethereum.NotFound,
			expectedErr: io.EOF,
		},
		{
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			src := testutils.NewMockL1Reader(ctrl)
			src.EXPECT().L1BlockRefByNumber(gomock.Any(), test.startBlock.Number+1).Return(test.nextBlock, test.fetcherErr).Times(1)
			...
			if test.l1Receipts != nil {
				src.EXPECT().FetchReceipts(gomock.Any(), test.nextBlock.Hash).Return(info, test.l1Receipts, nil).Times(1)
			}
