...
func (tx *testTx) Create(t *testing.T, signer l1types.Signer, rng *rand.Rand) *l1types.Transaction {
	t.Helper()
	out, err := l1types.SignNewTx(tx.author, signer, &l1types.DynamicFeeTx{
...
func TestDataFromEVMTransactions(t *testing.T) {
	...
	altInbox := testutils.RandomL1Address(rand.New(rand.NewSource(1234)))
	altAuthor := testutils.RandomKey()

	batchInboxAddress := l1common.Address(cfg.BatchInboxAddress)
	batcherAddress := l1common.Address(batcherAddr)
	testCases := []calldataTest{
		{
			name: "simple",
			txs:  []testTx{{to: &batchInboxAddress, dataLen: 1234, author: batcherPriv, good: true}},
		},
		{
			name: "other inbox",
		...
		{
			name: "other author",
			txs:  []testTx{{to: &batchInboxAddress, dataLen: 1234, author: altAuthor, good: false}},
		},
		{
			name: "inbox is author",
			txs:  []testTx{{to: &batchInboxAddress, dataLen: 1234, author: inboxPriv, good: false}},
		},
		{
			name: "author is inbox",
			txs:  []testTx{{to: &batcherAddress, dataLen: 1234, author: batcherPriv, good: false}},
		},
		{
			name: "unrelated",
		...
		{
			name: "empty tx",
			txs:  []testTx{{to: &batchInboxAddress, dataLen: 0, author: batcherPriv, good: true}},
		},
		{
			name: "value tx",
			txs:  []testTx{{to: &batchInboxAddress, dataLen: 1234, value: 42, author: batcherPriv, good: true}},
		},
		{
		...
		},
		{
			name: "mixed txs",
			txs: []testTx{
				{to: &batchInboxAddress, dataLen: 1234, value: 42, author: batcherPriv, good: true},
				{to: &batchInboxAddress, dataLen: 3333, value: 32, author: altAuthor, good: false},
				{to: &batchInboxAddress, dataLen: 2000, value: 22, author: batcherPriv, good: true},
