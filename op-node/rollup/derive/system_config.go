...
var (
	SystemConfigUpdateBatcher           = l1common.Hash{31: 0}
	SystemConfigUpdateGasConfig         = l1common.Hash{31: 1}
	SystemConfigUpdateGasLimit          = l1common.Hash{31: 2}
	SystemConfigUpdateUnsafeBlockSigner = l1common.Hash{31: 3}
)
...
var (
	...
	ConfigUpdateEventVersion0 = l1common.Hash{}
)

// UpdateSystemConfigWithL1Receipts filters all L1 receipts to find config updates and applies the config updates to the given sysCfg
func UpdateSystemConfigWithL1Receipts(sysCfg *eth.SystemConfig, receipts []*l1types.Receipt, cfg *rollup.Config, l1Time uint64) error {
	var result error
	for i, rec := range receipts {
		...
		for j, log := range rec.Logs {
			if log.Address == l1common.Address(cfg.L1SystemConfigAddress) && len(log.Topics) > 0 && log.Topics[0] == ConfigUpdateEventABIHash {
...
//	    bytes data
//	);
func ProcessSystemConfigUpdateLogEvent(destSysCfg *eth.SystemConfig, ev *l1types.Log, rollupCfg *rollup.Config, l1Time uint64) error {
	if len(ev.Topics) != 3 {
