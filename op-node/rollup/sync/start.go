...
var (
	...
	ErrTooDeepReorg   = errors.New("reorg is too deep")
)
...
func FindL2Heads(ctx context.Context, cfg *rollup.Config, l1 L1Chain, l2 L2Chain, lgr log.Logger, syncCfg *Config) (result *FindHeadsResult, err error) {
	...
	for {
		...
		if n.L1Origin.Hash == l1Block.ParentHash {
			...
			notFound := errors.Is(err, l1ethereum.NotFound)
			if err != nil && !notFound {
		...
		if result.Unsafe == (eth.L2BlockRef{}) {
			...
			if n.L1Origin.Number+(MaxReorgSeqWindows*cfg.SyncLookback()) < prevUnsafe.L1Origin.Number {
				...
				// stopgap solution.
				return nil, fmt.Errorf("%w: traversed back to L2 block %s, but too deep compared to previous unsafe block %s", ErrTooDeepReorg, n, prevUnsafe)
			}
		}
