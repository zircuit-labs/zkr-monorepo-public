...
type TransactionWithMetadata struct {
	...
	InboxAddr   l1common.Address   `json:"inbox_address"`
	BlockNumber uint64             `json:"block_number"`
	BlockHash   l1common.Hash      `json:"block_hash"`
	BlockTime   uint64             `json:"block_time"`
	ChainId     uint64             `json:"chain_id"`
	Sender      l1common.Address   `json:"sender"`
...
func Batches(client *l1ethclient.Client, beacon *l1.L1BeaconClient, config Config) (totalValid, totalInvalid uint64) {
...
func fetchBatchesPerBlock(ctx context.Context, client *l1ethclient.Client, beacon *l1.L1BeaconClient, number uint64, signer types.Signer, config Config) (uint64, uint64, error) {
		...
		if tx.To() != nil && *tx.To() == config.BatchInbox {
			...
			if tx.Type() != types.BlobTxType {
				...
				blobs, err := beacon.GetBlobs(ctx, eth.L1BlockRef{
					...
					ParentHash: l2common.Hash(block.ParentHash()),
					Time:       block.Time(),
				}, hashes)
				if err != nil {
