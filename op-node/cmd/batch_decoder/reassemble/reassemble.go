...
func processFrames(cfg Config, rollupCfg *rollup.Config, id derive.ChannelID, frames []FrameWithMetadata) ChannelWithMetadata {
	...
	if ch.IsReady() {
		br, err := derive.BatchReader(ch.Reader(), spec.MaxRLPBytesPerChannel(ch.HighestBlock().Time), false)
		if err == nil {
...
func loadTransactions(dir string, inbox common.Address) []fetch.TransactionWithMetadata {
	...
	for _, file := range files {
		...
		if (inbox == common.Address{} || txm.InboxAddr == l1common.Address(inbox)) && txm.ValidSender {
			out = append(out, txm)
		}
	}
	return out
