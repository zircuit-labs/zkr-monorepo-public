...
import (
	...
	l1event "github.com/ethereum/go-ethereum/event"
	...
	zkrretry "github.com/zircuit-labs/zkr-go-common/retry"
	"github.com/zircuit-labs/zkr-go-common/retry/strategy"

	"github.com/zircuit-labs/zkr-monorepo-public/op-node/heartbeat"
	"github.com/zircuit-labs/zkr-monorepo-public/op-node/metrics"
	"github.com/zircuit-labs/zkr-monorepo-public/op-node/node/safedb"
	...
	"github.com/zircuit-labs/zkr-monorepo-public/op-node/rollup/driver"
	"github.com/zircuit-labs/zkr-monorepo-public/op-node/rollup/status"
	"github.com/zircuit-labs/zkr-monorepo-public/op-node/rollup/sync"
	"github.com/zircuit-labs/zkr-monorepo-public/op-node/version"
	...
	"github.com/zircuit-labs/zkr-monorepo-public/op-service/oppprof"
	...
	l1 "github.com/zircuit-labs/zkr-monorepo-public/op-service/sources/l1"
...
type OpNode struct {
	...
	l1Source  l1.L1Reader           // L1 Client to fetch data from
	...
	// embedded nats
	embeddedNatsServer *messagebus.NatsEmbeddedServer
	natsConnection     *nats.Conn
	natsConfig         *config.Configuration
}
...
func New(ctx context.Context, cfg *Config, log log.Logger, appVersion string, m *metrics.Metrics) (*OpNode, error) {
	...
	if err := n.setupNats(cfg); err != nil {
		log.Error("Error initializing embedded nats server", "err", err)
		return nil, err
	}
...
func (n *OpNode) setupNats(cfg *Config) error {
	if cfg.NatsConfig == nil {
		log.Info("No embedded nats server configured")
		return nil
	}

	natsConfig, err := config.NewConfigurationFromMap(
		map[string]any{
			"servername":            "op_node_embedded",
			"jetstreamdomain":       "op_node_embedded",
			"leafnodeurl":           cfg.NatsConfig.LeafNodeURL,
			"storedir":              cfg.NatsConfig.StoreDir,
			"remotecredentialspath": cfg.NatsConfig.RemoteCredentialsPath,
			"subject":               "l2blocks",
			"stream":                "L2BLOCKS",
			"enablelogging":         cfg.NatsConfig.EnableLogging,
		},
	)
	if err != nil {
		return err
	}
	n.natsConfig = natsConfig

	natsServer, err := messagebus.NewNatsEmbeddedServer(natsConfig, "")
	if err != nil {
		return err
	}
	n.embeddedNatsServer = natsServer

	natsConnection, err := natsServer.NewConnection()
	if err != nil {
		n.log.Error("Error connecting to embedded nats server", "err", err)
		return err
	}
	n.natsConnection = natsConnection

	// ensure the stream is set up
	...
	_, err = js.CreateOrUpdateStream(context.Background(), jetstream.StreamConfig{
		Name:        "L2BLOCKS",
		Subjects:    []string{"l2blocks", "l2blocks.>"},
...
func (n *OpNode) initL1(ctx context.Context, cfg *Config) error {
	...
	rpcCfg.L1EthClientConfig.RethDBPath = cfg.RethDBPath

	l1Client, err := l1.NewL1Client(
		l1client.NewInstrumentedRPC(l1Node, &n.metrics.RPCMetrics.RPCClientMetrics), n.log, n.metrics.L1SourceCache, rpcCfg)
	if err != nil {
		return fmt.Errorf("failed to create L1 client: %w", err)
	}
	n.l1Source = l1.NewL1DataReader(l1Client, n.log)
	...
	n.l1HeadsSub = l1event.ResubscribeErr(time.Second*10, func(ctx context.Context, err error) (l1event.Subscription, error) {
		if err != nil {
			n.log.Warn("resubscribing after failed L1 subscription", "err", err)
		}
		return l1eth.WatchHeadChanges(ctx, n.l1Source, n.OnNewL1Head)
	})
	go func() {
	...
	n.l1SafeSub = l1eth.PollBlockChanges(n.log, n.l1Source, n.OnNewL1Safe, eth.Safe,
		cfg.L1EpochPollInterval, time.Second*10)
	n.l1FinalizedSub = l1eth.PollBlockChanges(n.log, n.l1Source, n.OnNewL1Finalized, eth.Finalized,
		cfg.L1EpochPollInterval, time.Second*10)
	return nil
}
...
func (n *OpNode) initL1BeaconAPI(ctx context.Context, cfg *Config) error {
	...
	beaconCfg := l1.L1BeaconClientConfig{
		FetchAllSidecars: cfg.Beacon.ShouldFetchAllSidecars(),
	}
	n.beacon = l1.NewL1BeaconClient(beaconClient, beaconCfg, fallbacks...)
...
func (n *OpNode) initL2(ctx context.Context, cfg *Config) error {
	...
	var l2BlockProducer status.L2BlockProducer
	if n.natsConnection != nil {
		producer, err := messagebus.NewNatsStreamProducer[types.L2Block](
			n.natsConfig,
			"",
			messagebus.WithNATSConnection(n.natsConnection),
			messagebus.WithRetrier(zkrretry.NewRetrier(
				zkrretry.WithMaxAttempts(3),
				zkrretry.WithStrategy(strategy.NewConstant(10*time.Millisecond)),
			)),
		)
		if err != nil {
			return fmt.Errorf("failed to create nats stream producer: %w", err)
		}
		producer.SetSubjectTransform(func(data types.L2Block, defaultSubject string) string {
			return fmt.Sprintf("%s.%s.%s", defaultSubject, data.Hash, data.Status)
		})
		l2BlockProducer = producer
	} else {
		l2BlockProducer = status.NilL2BlockProducer{}
	}
...
func (n *OpNode) Stop(ctx context.Context) error {
	...
	// Close the embedded nats server and connection to it
	if n.natsConnection != nil {
		n.natsConnection.Close()
	}
	if n.embeddedNatsServer != nil {
		n.embeddedNatsServer.Close()
	}
