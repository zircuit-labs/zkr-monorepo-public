...
var testEthClientConfig = &L1EthClientConfig{
...
func randHeader() (*types.Header, *RPCHeader) {
	...
	rhdr := &RPCHeader{
		...
		Bloom:       l1eth.Bytes256(hdr.Bloom),
...
func TestEthClient_InfoByHash(t *testing.T) {
	...
	s, err := NewL1EthClient(m, nil, nil, testEthClientConfig)
...
func newEthClientWithCaches(metrics caching.Metrics, cacheSize int) *L1EthClient {
	return &L1EthClient{
		transactionsCache: caching.NewLRUCache[common.Hash, types.Transactions](metrics, "txs", cacheSize),
		headersCache:      caching.NewLRUCache[common.Hash, l1eth.BlockInfo](metrics, "headers", cacheSize),
	}
}
