...
type ReceiptsProvider interface {
	...
	FetchReceipts(ctx context.Context, blockInfo l1eth.BlockInfo, txHashes []common.Hash) (types.Receipts, error)
}
...
func validateReceipts(block l1eth.BlockID, receiptHash common.Hash, txHashes []common.Hash, receipts []*types.Receipt) error {
	if len(receipts) != len(txHashes) {
		return fmt.Errorf("got %d receipts but expected %d", len(receipts), len(txHashes))
	}
	if len(txHashes) == 0 {
		if receiptHash != types.EmptyReceiptsHash {
