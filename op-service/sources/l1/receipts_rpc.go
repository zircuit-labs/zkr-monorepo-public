...
func newRPCRecProviderFromConfig(client client.RPC, log log.Logger, metrics caching.Metrics, config *L1EthClientConfig) *CachingReceiptsProvider {
...
func (f *RPCReceiptsFetcher) FetchReceipts(ctx context.Context, blockInfo l1eth.BlockInfo, txHashes []common.Hash) (result types.Receipts, err error) {
	m := f.PickReceiptsMethod(len(txHashes))
	block := l1eth.ToBlockID(blockInfo)
	switch m {
	...
	case DebugGetRawReceipts:
		...
		if err == nil {
			if len(rawReceipts) == len(txHashes) {
				result, err = l1eth.DecodeRawReceipts(block, rawReceipts, txHashes)
			} else {
...
const (
	...
	// This since has been standardized in https://github.com/zircuit-labs/execution-apis/pull/438 and adopted in Geth:
	// https://github.com/zircuit-labs/l2-geth-public/pull/27702
	// Available in:
