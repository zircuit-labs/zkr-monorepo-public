...
import (
	...
	"github.com/zircuit-labs/zkr-monorepo-public/op-service/sources/l1/eth"
	l1testutils "github.com/zircuit-labs/zkr-monorepo-public/op-service/testutils/l1"
)
...
func (tc *ReceiptsTestCase) Run(t *testing.T) {
	...
	ethCl, err := NewL1EthClient(client.NewBaseRPCClient(cl), logger, nil, testCfg)
	require.NoError(t, err)
	defer ethCl.Close()
...
func randomRpcBlockAndReceipts(rng *rand.Rand, txCount uint64) (*RPCBlock, []*types.Receipt) {
	block, receipts := l1testutils.RandomBlock(rng, txCount)
	return &RPCBlock{
		RPCHeader: RPCHeader{
...
func TestVerifyReceipts(t *testing.T) {
	...
	t.Run("NoTxWithEmptyTrieRoot", func(t *testing.T) {
		...
		err := validateReceipts(block, types.EmptyReceiptsHash, nil, nil)
		require.NoError(t, err)
	})
