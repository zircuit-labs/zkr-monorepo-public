...
type HeadSignalFn func(ctx context.Context, sig l2eth.L1BlockRef)
	...
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			case header := <-headChanges:
				fn(eventsCtx, l2eth.L1BlockRef{
					Hash:       ConvertHashToL2(header.Hash()),
					Number:     header.Number.Uint64(),
					ParentHash: ConvertHashToL2(header.ParentHash),
					Time:       header.Time,
				})
			case <-eventsCtx.Done():
				return nil
...
func PollBlockChanges(log log.Logger, src L1BlockRefsSource, fn HeadSignalFn,
	label l2eth.BlockLabel, interval time.Duration, timeout time.Duration,
