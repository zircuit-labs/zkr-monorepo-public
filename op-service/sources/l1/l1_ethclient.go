...
func (c *L1EthClientConfig) Check() error {
...
// L1EthClient retrieves ethereum data with optimized batch requests, cached results, and flag to not trust the RPC.
type L1EthClient struct {
	client client.RPC
	...
	headersCache *caching.LRUCache[common.Hash, l1eth.BlockInfo]
}

// NewEthClient returns an [L1EthClient], wrapping an RPC with bindings to fetch ethereum data with added error logging,
// metric tracking, and caching. The [L1EthClient] uses a [LimitRPC] wrapper to limit the number of concurrent RPC requests.
func NewL1EthClient(client client.RPC, log log.Logger, metrics caching.Metrics, config *L1EthClientConfig) (*L1EthClient, error) {
	...
	return &L1EthClient{
		...
		headersCache:      caching.NewLRUCache[common.Hash, l1eth.BlockInfo](metrics, "headers", config.HeadersCacheSize),
	}, nil
}

// SubscribeNewHead subscribes to notifications about the current blockchain head on the given channel.
func (s *L1EthClient) SubscribeNewHead(ctx context.Context, ch chan<- *types.Header) (ethereum.Subscription, error) {
...
// l1RpcBlockID is an internal type to enforce header and block call results match the requested identifier
type l1RpcBlockID interface {
	...
	CheckID(id l1eth.BlockID) error
}

// hashID implements l1RpcBlockID for safe block-by-hash fetching
type l1HashID common.Hash

func (h l1HashID) Arg() any { return common.Hash(h) }
func (h l1HashID) CheckID(id l1eth.BlockID) error {
...
// l1eth.NumberID implements l1RpcBlockID for safe block-by-number fetching
type l1NumberID uint64

func (n l1NumberID) Arg() any { return hexutil.EncodeUint64(uint64(n)) }
func (n l1NumberID) CheckID(id l1eth.BlockID) error {
...
func (s *L1EthClient) headerCall(ctx context.Context, method string, id l1RpcBlockID) (l1eth.BlockInfo, error) {
	...
	if err := id.CheckID(l1eth.ToBlockID(info)); err != nil {
...
func (s *L1EthClient) blockCall(ctx context.Context, method string, id l1RpcBlockID) (l1eth.BlockInfo, types.Transactions, error) {
	var block *RPCBlock
...
func (s *L1EthClient) ChainID(ctx context.Context) (*big.Int, error) {
	var id hexutil.Big
...
func (s *L1EthClient) InfoByHash(ctx context.Context, hash common.Hash) (l1eth.BlockInfo, error) {
	if header, ok := s.headersCache.Get(hash); ok {
		return header, nil
	}
	return s.headerCall(ctx, "eth_getBlockByHash", l1eth.HashID(hash))
}

func (s *L1EthClient) InfoByNumber(ctx context.Context, number uint64) (l1eth.BlockInfo, error) {
	// can't hit the cache when querying by number due to reorgs.
	return s.headerCall(ctx, "eth_getBlockByNumber", l1eth.NumberID(number))
}

func (s *L1EthClient) InfoByLabel(ctx context.Context, label l1eth.BlockLabel) (l1eth.BlockInfo, error) {
	// can't hit the cache when querying the head due to reorgs / changes.
	return s.headerCall(ctx, "eth_getBlockByNumber", label)
}

func (s *L1EthClient) InfoAndTxsByHash(ctx context.Context, hash common.Hash) (l1eth.BlockInfo, types.Transactions, error) {
	if header, ok := s.headersCache.Get(hash); ok {
		if txs, ok := s.transactionsCache.Get(hash); ok {
			return header, txs, nil
		}
	}
	return s.blockCall(ctx, "eth_getBlockByHash", l1eth.HashID(hash))
}

func (s *L1EthClient) InfoAndTxsByNumber(ctx context.Context, number uint64) (l1eth.BlockInfo, types.Transactions, error) {
	// can't hit the cache when querying by number due to reorgs.
	return s.blockCall(ctx, "eth_getBlockByNumber", l1eth.NumberID(number))
}

func (s *L1EthClient) InfoAndTxsByLabel(ctx context.Context, label l1eth.BlockLabel) (l1eth.BlockInfo, types.Transactions, error) {
	// can't hit the cache when querying the head due to reorgs / changes.
...
func (s *L1EthClient) FetchReceipts(ctx context.Context, blockHash common.Hash) (l1eth.BlockInfo, types.Receipts, error) {
	info, txs, err := s.InfoAndTxsByHash(ctx, blockHash)
	if err != nil {
		return nil, nil, fmt.Errorf("querying block: %w", err)
	}

	txHashes, _ := l1eth.TransactionsToHashes(txs), l1eth.ToBlockID(info)
...
func (s *L1EthClient) GetProof(ctx context.Context, address common.Address, storage []common.Hash, blockTag string) (*l1eth.AccountResult, error) {
	var getProofResponse *l1eth.AccountResult
...
func (s *L1EthClient) GetStorageAt(ctx context.Context, address common.Address, storageSlot common.Hash, blockTag string) (common.Hash, error) {
	var out common.Hash
...
func (s *L1EthClient) ReadStorageAt(ctx context.Context, address common.Address, storageSlot common.Hash, blockHash common.Hash) (common.Hash, error) {
	if s.trustRPC {
		hash, err := s.GetStorageAt(ctx, address, storageSlot, blockHash.String())
		return hash, err
	}
