...
type Lifecycle interface {
	...
	Stopped() bool
	// Name is used for telemetry and profiling.
	Name() string
}
...
func LifecycleCmd(fn LifecycleAction) cli.ActionFunc {
	return func(ctx *cli.Context) error {
		...
		if _, ok := os.LookupEnv("DD_APM_ENABLED"); ok {
			err = profiler.Start(
				profiler.WithService(appLifecycle.Name()),
			...
			if err != nil {
				return errors.Join(
					fmt.Errorf("failed to start profiler: %w", err),
					context.Cause(appCtx),
				)
			}
			defer profiler.Stop()
			...
			tracer.Start(
				tracer.WithService(appLifecycle.Name()),
			)
			defer tracer.Stop()
		}
