...
func (m *MockEthClient) FetchReceipts(ctx context.Context, blockHash common.Hash) (eth.BlockInfo, l1types.Receipts, error) {
	out := m.Mock.Called(blockHash)
	return *out.Get(0).(*eth.BlockInfo), out.Get(1).(l1types.Receipts), out.Error(2)
}

func (m *MockEthClient) ExpectFetchReceipts(hash common.Hash, info eth.BlockInfo, receipts l1types.Receipts, err error) {
...
func (m *MockEthClient) BlockByNumberEx(ctx context.Context, number *big.Int) (*ethclient.BlockEx, error) {
	out := m.Mock.Called(number)
	return out.Get(0).(*ethclient.BlockEx), out.Error(1)
}

func (m *MockEthClient) ExpectBlockByNumberEx(number *big.Int, block *ethclient.BlockEx, err error) {
	m.Mock.On("BlockByNumberEx", number).Once().Return(block, err)
}
