...
func RandomL1Hash(rng *rand.Rand) (out l1common.Hash) {
	rng.Read(out[:])
	return
}
...
func RandomL1Address(rng *rand.Rand) (out l1common.Address) {
	rng.Read(out[:])
	return
}
...
func RandomDepositTx(rng *rand.Rand) *types.Transaction { // TODO check usages
	to := RandomAddress(rng)
	return types.NewTx(&types.DepositTx{
		SourceHash:          RandomHash(rng),
		From:                RandomAddress(rng),
		To:                  &to,
		Value:               big.NewInt(int64(rng.Int())),
		Gas:                 25000 + uint64(rng.Intn(40000)),
