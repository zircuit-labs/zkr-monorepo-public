...
import (
	...
	testutils "github.com/zircuit-labs/zkr-monorepo-public/op-service/testutils"
)
...
func InsecureRandomKey(rng *rand.Rand) *ecdsa.PrivateKey {
	idx := rng.Intn(len(testutils.RandomEcdsaKeys))
	key, err := crypto.ToECDSA(common.FromHex(testutils.RandomEcdsaKeys[idx]))
	if err != nil {
