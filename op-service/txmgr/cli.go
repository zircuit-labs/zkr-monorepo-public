...
import (
	...
	opcrypto "github.com/zircuit-labs/zkr-monorepo-public/op-service/crypto"
	"github.com/zircuit-labs/zkr-monorepo-public/op-service/eth"
	opsigner "github.com/zircuit-labs/zkr-monorepo-public/op-service/signer"
)
...
const (
	...
	SignerTypeFlagName               = "signer-type"
	KeystoreFileFlagName             = "s3keystore-file"
	KeystorePassphraseFlagName       = "s3keystore-passphrase"
	KeystoreEndpointFlagName         = "s3keystore-endpoint"
	KeystoreAccessKeyIDFlagName      = "s3keystore-accesskeyid"
	KeystoreSecretAccessKeyFlagName  = "s3keystore-secretaccesskey"
	KeystoreBucketFlagName           = "s3keystore-bucket"
	KeystoreRegionFlagName           = "s3keystore-region"
	KeystoreS3ForcePathStyleFlagName = "s3keystore-s3forcepathstyle"
	KeystoreDisableSSLFlagName       = "s3keystore-disablessl"
	...
	SafeAbortStuckInGasFeeIncreaseLoop = "safe-abort-stuck-in-gas-fee-increase-loop"
...
type DefaultFlagValues struct {
	...
	SafeAbortStuckInGasFeeIncreaseLoop uint64
...
var (
	DefaultBatcherFlagValues = DefaultFlagValues{
		...
		SafeAbortStuckInGasFeeIncreaseLoop: uint64(20),
...
func CLIFlagsWithDefaults(envPrefix string, defaults DefaultFlagValues) []cli.Flag {
	...
	return append([]cli.Flag{
		&cli.StringFlag{
			Name:    SignerTypeFlagName,
			Usage:   "The signer type used to derive the wallets for the service",
			EnvVars: prefixEnvVars("SIGNER_TYPE"),
		},
		&cli.StringFlag{
			Name:    KeystoreFileFlagName,
			Usage:   "The path to the keystore file",
			EnvVars: prefixEnvVars("S3KEYSTORE_FILE"),
		},
		&cli.StringFlag{
			Name:    KeystorePassphraseFlagName,
			Usage:   "The passphrase to decrypt the keystore file",
			EnvVars: prefixEnvVars("S3KEYSTORE_PASSPHRASE"),
		},
		&cli.StringFlag{
			Name:    KeystoreEndpointFlagName,
			Usage:   "The endpoint for the s3 keystore",
			EnvVars: prefixEnvVars("S3KEYSTORE_ENDPOINT"),
		},
		&cli.StringFlag{
			Name:    KeystoreAccessKeyIDFlagName,
			Usage:   "The access key id for the s3 keystore",
			EnvVars: prefixEnvVars("S3KEYSTORE_ACCESSKEYID"),
		},
		&cli.StringFlag{
			Name:    KeystoreSecretAccessKeyFlagName,
			Usage:   "The secret access key for the s3 keystore",
			EnvVars: prefixEnvVars("S3KEYSTORE_SECRETACCESSKEY"),
		},
		&cli.StringFlag{
			Name:    KeystoreBucketFlagName,
			Usage:   "The bucket for the s3 keystore",
			EnvVars: prefixEnvVars("S3KEYSTORE_BUCKET"),
		},
		&cli.StringFlag{
			Name:    KeystoreRegionFlagName,
			Usage:   "The region for the s3 keystore",
			EnvVars: prefixEnvVars("S3KEYSTORE_REGION"),
		},
		&cli.BoolFlag{
			Name:    KeystoreS3ForcePathStyleFlagName,
			Usage:   "The s3 force path style for the s3 keystore",
			EnvVars: prefixEnvVars("S3KEYSTORE_S3FORCEPATHSTYLE"),
		},
		&cli.BoolFlag{
			Name:    KeystoreDisableSSLFlagName,
			Usage:   "The disable ssl for the s3 keystore",
			EnvVars: prefixEnvVars("S3KEYSTORE_DISABLESSL"),
		},
		&cli.StringFlag{
		...
		&cli.Uint64Flag{
			Name:    SafeAbortStuckInGasFeeIncreaseLoop,
			Usage:   "Number of unable to increase gas / bumping gas price for transaction loop observations required to give up on a TX without receiving confirmation",
			Value:   defaults.SafeAbortStuckInGasFeeIncreaseLoop,
			EnvVars: prefixEnvVars("SAFE_ABORT_STUCK_IN_GAS_FEE_INCREASE_LOOP"),
		},
		&cli.Uint64Flag{
...
type CLIConfig struct {
	...
	// zr changes
	SignerType       opcrypto.SignerType
	S3KeystoreConfig opcrypto.S3KeystoreConfig
}
...
func NewCLIConfig(l1RPCURL string, defaults DefaultFlagValues) CLIConfig {
	return CLIConfig{
		...
		SafeAbortStuckInGasFeeIncreaseLoop: defaults.SafeAbortStuckInGasFeeIncreaseLoop,
...
func (m CLIConfig) Check() error {
	...
	if m.SafeAbortStuckInGasFeeIncreaseLoop == 0 {
		return errors.New("SafeAbortStuckInGasFeeIncreaseLoop must not be 0")
	}
...
func ReadCLIConfig(ctx *cli.Context) CLIConfig {
	signerType := opcrypto.Mnemonic
	if ctx.IsSet(SignerTypeFlagName) {
		parsedType, err := opcrypto.ParseSignerType(ctx.String(SignerTypeFlagName))
		if err != nil {
			panic(fmt.Errorf("could not parse signer type: %w", err))
		}
		signerType = parsedType
	}

	var ksConfig opcrypto.S3KeystoreConfig
	if ctx.IsSet(KeystorePassphraseFlagName) && ctx.IsSet(KeystoreFileFlagName) {
		ksConfig = opcrypto.S3KeystoreConfig{
			S3File:     ctx.String(KeystoreFileFlagName),
			Passphrase: ctx.String(KeystorePassphraseFlagName),
			BlobStoreConfig: s3.BlobStoreConfig{
				Endpoint:         ctx.String("s3keystore-endpoint"),
				AccessKeyID:      ctx.String("s3keystore-accesskeyid"),
				SecretAccessKey:  ctx.String("s3keystore-secretaccesskey"),
				Bucket:           ctx.String("s3keystore-bucket"),
				Region:           ctx.String("s3keystore-region"),
				S3ForcePathStyle: ctx.Bool("s3keystore-s3forcepathstyle"),
				DisableSSL:       ctx.Bool("s3keystore-disablessl"),
			},
		}
	}
	return CLIConfig{
		...
		SafeAbortStuckInGasFeeIncreaseLoop: ctx.Uint64(SafeAbortStuckInGasFeeIncreaseLoop),
...
func NewConfig(cfg CLIConfig, l log.Logger) (Config, error) {
	...
	signerFactory, from, err := opcrypto.SignerFactoryFromConfig(
		l,
		cfg.PrivateKey,
		cfg.Mnemonic,
		hdPath,
		cfg.SignerType,
		cfg.S3KeystoreConfig,
	)
	if err != nil {
	...
	return Config{
		...
		SafeAbortStuckInGasFeeIncreaseLoop: cfg.SafeAbortStuckInGasFeeIncreaseLoop,
