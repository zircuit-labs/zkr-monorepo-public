...
const (
	testSafeAbortNonceTooLowCount          = 3
	testSafeAbortStuckInGasFeeIncreaseLoop = 20
)
...
func newSendStateWithTimeout(t time.Duration, now func() time.Time) *SendState {
	return NewSendStateWithNow(testSafeAbortNonceTooLowCount, t, now, testSafeAbortStuckInGasFeeIncreaseLoop)
}
...
func simulateIncreaseFailedBumpCount(sendState *SendState, n int) {
	for i := 0; i < n; i++ {
...
func TestCriticalErrorIfGasPriceBumpRetriedTooOften(t *testing.T) {
	sendState := newSendState()
	simulateIncreaseFailedBumpCount(sendState, testSafeAbortStuckInGasFeeIncreaseLoop+1)
	require.Error(t, sendState.CriticalError(), ErrStuckInFailingToIncreaseGasPriceLoop)
}

func TestNoAbortIfNotStuckInIncreasingGasPriceLoopYet(t *testing.T) {
	sendState := newSendState()
	simulateIncreaseFailedBumpCount(sendState, testSafeAbortStuckInGasFeeIncreaseLoop-1)
	require.Nil(t, sendState.CriticalError())
}
