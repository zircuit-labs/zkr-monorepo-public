...
var (
	...
	ErrStuckInFailingToIncreaseGasPriceLoop = errors.New("got stuck in loop of failing to bump gas price")
)
...
type SendState struct {
	...
	failedBumpCounts int // additionally track how often it failed to bump to detect safeAbortStuckInGasFeeIncreaseLoop
}

// NewSendStateWithNow creates a new send state with the provided clock.
func NewSendStateWithNow(safeAbortNonceTooLowCount uint64, unableToSendTimeout time.Duration, now func() time.Time, safeAbortStuckInGasFeeIncreaseLoop uint64) *SendState {
	if safeAbortNonceTooLowCount == 0 {
		panic("txmgr: safeAbortNonceTooLowCount cannot be zero")
	}
	if safeAbortStuckInGasFeeIncreaseLoop == 0 {
		panic("txmgr: safeAbortStuckInGasFeeIncreaseLoop cannot be zero")
	}

	return &SendState{
		minedTxs:                           make(map[common.Hash]struct{}),
		safeAbortNonceTooLowCount:          safeAbortNonceTooLowCount,
		safeAbortStuckInGasFeeIncreaseLoop: safeAbortStuckInGasFeeIncreaseLoop,
...
func NewSendState(safeAbortNonceTooLowCount uint64, unableToSendTimeout time.Duration, safeAbortStuckInGasFeeIncreaseLoop uint64) *SendState {
	return NewSendStateWithNow(safeAbortNonceTooLowCount, unableToSendTimeout, time.Now, safeAbortStuckInGasFeeIncreaseLoop)
}
...
func (s *SendState) CriticalError() error {
	...
	switch {
	// Gas Fee was bumped, or at least tried to be bumped that we abort the TX and retry it from one level higher
	case s.failedBumpCounts > int(s.safeAbortStuckInGasFeeIncreaseLoop):
		return ErrStuckInFailingToIncreaseGasPriceLoop
