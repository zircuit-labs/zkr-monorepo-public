...
import (
	...
	"github.com/zircuit-labs/zkr-monorepo-public/op-service/txmgr/metrics"
)
...
func testSendState() *SendState {
	return NewSendState(100, time.Hour, 20)
}
...
func TestWaitMinedCanBeCanceled(t *testing.T) {
	...
	receipt, err := h.mgr.waitMined(ctx, tx, NewSendState(10, time.Hour, 20))
	require.Equal(t, err, context.DeadlineExceeded)
	require.Nil(t, receipt)
}
...
func TestIncreaseGasPrice(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			run: func(t *testing.T) {
				_, newTx, err := doGasPriceIncrease(t, 1, 9999, 1, 1)
				require.True(t, newTx.GasTipCap().Cmp(big.NewInt(1)) == 0, "new tx tip must equal to suggest prices")
				require.True(t, newTx.GasFeeCap().Cmp(big.NewInt(3)) == 0, "new tx fee cap must be derived from suggest prices")
				require.NoError(t, err)
			},
		},
		{
			...
			run: func(t *testing.T) {
				_, newTx, err := doGasPriceIncrease(t, 9999, 0, 0, 9999)
				require.True(t, newTx.GasTipCap().Cmp(big.NewInt(0)) == 0, "new tx tip must equal to suggest prices")
				require.True(t, newTx.GasFeeCap().Cmp(big.NewInt(19998)) == 0, "new tx fee cap must be derived from suggest prices")
				require.NoError(t, err)
			},
		},
		{
			...
			run: func(t *testing.T) {
				_, newTx, err := doGasPriceIncrease(t, 9999, 9999, 1, 1)
				require.True(t, newTx.GasTipCap().Cmp(big.NewInt(1)) == 0, "new tx tip must equal to suggest prices")
				require.True(t, newTx.GasFeeCap().Cmp(big.NewInt(3)) == 0, "new tx fee cap must be derived from suggest prices")
				require.NoError(t, err)
			},
...
// same, repeated calls to IncreaseGasPrice eventually hit a limit and reset.
...
// testIncreaseGasPriceLimit runs a gas bumping test that increases the gas price until it hits a reset.
...
func testIncreaseGasPriceLimit(t *testing.T, lt gasPriceLimitTest) {
	...
	for {
		...
		tmpTx, _ = mgr.increaseGasPrice(ctx, lastGoodTx)
		if lastGoodTx.GasFeeCap().Cmp(tmpTx.GasFeeCap()) > 0 {
			lastGoodTx = tmpTx
			break
		}
		lastGoodTx = tmpTx
	...
	// Confirm that fees were reset at expected threshold
	...
	for {
		var tmpTx *types.Transaction
		tmpTx, _ = mgr.increaseGasPrice(ctx, lastGoodTx)
		if lastGoodTx.BlobGasFeeCap().Cmp(tmpTx.BlobGasFeeCap()) > 0 {
			lastGoodTx = tmpTx
			break
		}
		lastGoodTx = tmpTx
