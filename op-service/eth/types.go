...
type PayloadAttributes struct {
	...
	// Zircuit addition: L1 info for the payload
	L1Info *types.L1Info `json:"l1Info,omitempty"`
}
...
const (
	...
	ExecutionSlsRejected ExecutePayloadStatus = "SLSREJECTED"
	// CCCRejected is returned if the payload fails CCC's check
	ExecutionCCCRejected ExecutePayloadStatus = "CCCREJECTED"
)
...
type PayloadStatusV1 struct {
	...
	// if available, the transactions should be checked and potentially rejected if malicious
...
type ForkchoiceUpdatedResult struct {
	...
	// the list of rejected deposit transactions
	RejectedTransactions []common.Hash `json:"rejectedTransactions,omitempty"`
}
...
func DecodeScalar(scalar [32]byte) (EcotoneScalars, error) {
	...
	default:
		return EcotoneScalars{}, fmt.Errorf("unexpected system config scalar: %s", scalar)
	}
}
