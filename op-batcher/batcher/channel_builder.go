...
func (c *ChannelBuilder) Blocks() []*ChannelBlock {
	return c.blocks
}
...
func (c *ChannelBuilder) AddBlock(block *types.Block, l1Info *derive.L1BlockInfo) error {
	if c.IsFull() {
		return c.FullErr()
	}

	batch, err := derive.BlockToSingularBatch(&c.rollupCfg, block, l1Info)
	if err != nil {
	...
	c.blocks = append(c.blocks, &ChannelBlock{
		block:  block,
		l1info: l1Info,
	})
