...
func (s *channelManager) processBlocks() error {
	...
	for i, block := range s.blocks {
		block, l1info := block.block, block.l1info
		err := s.currentChannel.AddBlock(block, l1info)
...
func (s *channelManager) AddL2Block(block *types.Block, l1Info *derive.L1BlockInfo) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	s.blocks = append(s.blocks, &ChannelBlock{
		block:  block,
		l1info: l1Info,
	})
	s.tip = block.Hash()
