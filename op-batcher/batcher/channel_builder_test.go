...
import (
	...
	dtest "github.com/zircuit-labs/zkr-monorepo-public/op-node/rollup/derive/test"
...
func addMiniBlock(cb *ChannelBuilder) error {
	a := newMiniL2Block(0)
	l1Info, _ := derive.L1BlockInfoFromBytes(&cb.rollupCfg, a.Time(), a.Transactions()[0].Data())
	err := cb.AddBlock(a, l1Info)
	return err
}
...
func newMiniL2BlockWithNumberParentAndL1Information(numTx int, l2Number *big.Int, parent common.Hash, l1Number int64, blockTime uint64) *types.Block {
	...
	l1InfoTx, err := derive.L1InfoDeposit(&defaultTestRollupConfig, eth.SystemConfig{}, 0, eth.BlockToInfo(l1Block), blockTime, &types.Bitmap{})
	if err != nil {
		panic(err)
	}
...
func addTooManyBlocks(cb *ChannelBuilder, blockCount int) (int, error) {
	...
	for i := 0; i < blockCount; i++ {
		...
		l1Info, _ := derive.L1BlockInfoFromBytes(&cb.rollupCfg, block.Time(), block.Transactions()[0].Data())
		err := cb.AddBlock(block, l1Info)
		if err != nil {
			return i + 1, err
		}
	}
...
func TestChannelBuilder_LatestL1Origin(t *testing.T) {
	...
	block := newMiniL2BlockWithNumberParentAndL1Information(0, big.NewInt(1), common.Hash{}, 1, 100)
	l1Info, err := derive.L1BlockInfoFromBytes(&cb.rollupCfg, block.Time(), block.Transactions()[0].Data())
	require.NoError(t, err)
	...
	block = newMiniL2BlockWithNumberParentAndL1Information(0, big.NewInt(2), common.Hash{}, 1, 100)
	l1Info, err = derive.L1BlockInfoFromBytes(&cb.rollupCfg, block.Time(), block.Transactions()[0].Data())
	require.NoError(t, err)
	...
	block = newMiniL2BlockWithNumberParentAndL1Information(0, big.NewInt(3), common.Hash{}, 2, 110)
	l1Info, err = derive.L1BlockInfoFromBytes(&cb.rollupCfg, block.Time(), block.Transactions()[0].Data())
	require.NoError(t, err)
	...
	block = newMiniL2BlockWithNumberParentAndL1Information(0, big.NewInt(3), common.Hash{}, 1, 110)
	l1Info, err = derive.L1BlockInfoFromBytes(&cb.rollupCfg, block.Time(), block.Transactions()[0].Data())
	require.NoError(t, err)
...
func ChannelBuilder_InputBytes(t *testing.T, batchType uint) {
	...
	for i := 0; i < 5; i++ {
		...
		if batchType == derive.SingularBatchType {
			...
			singularBatch, err := derive.BlockToSingularBatch(&defaultTestRollupConfig, block, l1Info)
			require.NoError(err)
...
func blockBatchRlpSize(t *testing.T, b *types.Block) int {
	t.Helper()
	l1Info, _ := derive.L1BlockInfoFromBytes(&defaultTestRollupConfig, b.Time(), b.Transactions()[0].Data())
	singularBatch, err := derive.BlockToSingularBatch(&defaultTestRollupConfig, b, l1Info)
