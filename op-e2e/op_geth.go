...
func (d *OpGeth) AddL2Block(ctx context.Context, txs ...*types.Transaction) (*eth.ExecutionPayloadEnvelope, error) {
	...
	if envelope.ExecutionErr != nil {
		return nil, errors.New(envelope.ExecutionErr.Err)
	}
...
func (d *OpGeth) CreatePayloadAttributes(txs ...*types.Transaction) (*eth.PayloadAttributes, error) {
	...
	l1Info, err := derive.L1InfoDepositBytes(d.l2Engine.RollupConfig(), d.SystemConfig, d.sequenceNum, d.L1Head, nil, uint64(timestamp))
	if err != nil {
		return nil, err
	}
	...
	if d.L2ChainConfig.IsCancun(new(big.Int).SetUint64(uint64(d.L2Head.BlockNumber)), uint64(timestamp)) {
