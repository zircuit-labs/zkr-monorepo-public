...
import (
	...
	gnode "github.com/zircuit-labs/l2-geth-public/node"
	...
	"github.com/zircuit-labs/zkr-monorepo-public/op-node/rollup/attributes"
	"github.com/zircuit-labs/zkr-monorepo-public/op-node/rollup/clsync"
	...
	"github.com/zircuit-labs/zkr-monorepo-public/op-service/safego"
...
func NewL2Verifier(t Testing, log log.Logger, l1 derive.L1Fetcher, blobsSrc derive.L1BlobsFetcher, eng L2API, cfg *rollup.Config, syncCfg *sync.Config, safeHeadListener safeDB) *L2Verifier {
	...
	pipeline := derive.NewDerivationPipeline(log, cfg, l1, blobsSrc, eng, metrics)
	sys.Register("pipeline", derive.NewPipelineDeriver(ctx, pipeline), opts)

	testActionEmitter := sys.Register("test-action", nil, opts)

	syncStatusTracker := status.NewStatusTracker(log, metrics, status.NilL2BlockProducer{})
...
func (s *L2Verifier) OnEvent(ev event.Event) bool {
	...
	case rollup.EngineTemporaryErrorEvent:
		...
		if errors.Is(x.Err, sync.ErrWrongChain) { // action-tests don't back off on temporary errors. Avoid a bad genesis setup from looping.
...
func (s *L2Verifier) ActL2InsertUnsafePayload(payload *eth.ExecutionPayloadEnvelope) Action {
	return func(t Testing) {
		ref, err := derive.PayloadToBlockRef(s.rollupCfg, payload.ExecutionPayload, payload.L1Info)
		require.NoError(t, err)
