...
type ChannelOutIface interface {
	ID() derive.ChannelID
	Reset() error
	AddBlock(rollupCfg *rollup.Config, block *types.Block, l1Info *derive.L1BlockInfo) error
	ReadyBytes() int
	Flush() error
	Close() error
...
// should be made.
func (co *GarbageChannelOut) AddBlock(rollupCfg *rollup.Config, block *types.Block, l1Info *derive.L1BlockInfo) error {
	if co.closed {
