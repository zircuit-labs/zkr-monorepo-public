...
SCRIPT_NAME="ApplyL2ThrottlingConfig.s.sol"
CONTRACT_NAME="L2ThrottlingScript"
# L2Controller
THROTTLING_ADMIN="0x4200000000000000000000000000000000000100"
SENDER=$(cast call "${THROTTLING_ADMIN}" "defaultAdmin()(address)" --rpc-url "${L2_RPC}")
...
forge script -vvvv "scripts/throttling/${SCRIPT_NAME}:${CONTRACT_NAME}" \
  --rpc-url "${L2_RPC}" --sender "${SENDER}" \
  --sig 'runSimulateFromMultisig(string memory)' "${DEPLOYMENT_NAME}"

FOUNDRY_OUTPUT_PATH="${SCRIPT_DIR}/../../broadcast/${SCRIPT_NAME}/$(cast chain-id --rpc-url ${L2_RPC})/dry-run/runSimulateFromMultisig-latest.json"
OUTPUT_PATH="${SCRIPT_DIR}/input_l2.json"
