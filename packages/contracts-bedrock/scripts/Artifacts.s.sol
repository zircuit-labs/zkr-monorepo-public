...
abstract contract Artifacts {
    ...
    function getAddress(string memory _name) public view returns (address payable) {
        ...
        } else if (digest == keccak256(bytes("L2Controller"))) {
            return payable(Predeploys.L2_CONTROLLER);
        }
        return payable(address(0));
    }
    ...
    function loadInitializedSlot(string memory _contractName) public view returns (uint64 initialized_) {
        ...
        bytes32 slotVal = vm.load(contractAddress, slot);
        initialized_ = uint64(uint256(slotVal));
    }
    ...
    function _getDeploymentContext() private view returns (string memory) {
        ...
        if (chainid == Chains.Mainnet) {
            ...
            return "sepolia";
        } else if (chainid == Chains.SepoliaAlphanet) {
            return "alphanet-sepolia";
        } else if (chainid == Chains.SepoliaBetanet) {
            return "betanet-sepolia";
        } else if (chainid == Chains.SepoliaBetanet2) {
            return "betanet2-sepolia";
        } else if (chainid == Chains.SepoliaTestnet) {
