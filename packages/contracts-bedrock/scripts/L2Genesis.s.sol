...
contract L2Genesis is Deployer {
    ...
    function setPredeployImplementations(L1Dependencies memory _l1Dependencies) internal {
        ...
        // 1,2,3,4,5: legacy, not used in OP-Stack.
    ...
    function setL2ToL1MessagePasser() public {
        address payable impl = payable(_setImplementationCode(Predeploys.L2_TO_L1_MESSAGE_PASSER));

        L2ToL1MessagePasser(impl).initialize();

        L2ToL1MessagePasser(payable(Predeploys.L2_TO_L1_MESSAGE_PASSER)).initialize();
    }
    ...
    /// @notice This predeploy is following the safety invariant #1.
    function setL2Controller() public {
        address impl = _setImplementationCode(Predeploys.L2_CONTROLLER);

        L2Controller(impl).initialize({
            _admin: address(0xdead),
            _paused: true
        });

        L2Controller(Predeploys.L2_CONTROLLER).initialize({
            _admin: cfg.superchainConfigGuardian(),
            _paused: false
        });
    }
