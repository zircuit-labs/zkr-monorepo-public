...
library ForgeArtifacts {
    ...
    /// @notice returns the constant slot of the Initializable storage
    function getInitializedSlot() internal pure returns (bytes32 slot_) {
        slot_ = keccak256(abi.encode(uint256(keccak256("openzeppelin.storage.Initializable")) - 1)) & ~bytes32(uint256(0xff));
    }
    ...
    function isInitialized(address _address) internal view returns (bool initialized_) {
        ...
        bytes32 slotVal = vm.load(_address, slot);
        initialized_ = uint64(uint256(slotVal)) != 0;
    }
