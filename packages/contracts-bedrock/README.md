...
[![codecov](https://codecov.io/gh/github.com/zircuit-labs/zkr-monorepo-public/branch/develop/graph/badge.svg?token=0VTG7PG7YR&flag=contracts-bedrock-tests)](https://codecov.io/gh/ethereum-optimism/optimism)
...
## Zircuit End-to-End Test

To run against devnet, spin up devnet using make `devnet-up`. Then navigate into `packages/contracts-bedrock` and run

```
mnemonic="test test test test test test test test test test test junk" \
rpcL1="http://localhost:8545" \
rpcL2="http://localhost:9545" \
deployment="devnetL1" \
node zr-e2e-test/test.js
```

You can use any other deployment that has a matching docker stack. Just ensure that the account on the mnemonic
have a few ETH on the refrenced L1 so that they can transact.
...
| ---------------------------------------------------------------------------------------- | ----------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| [`L1CrossDomainMessenger`](../../specs/messengers.md)                                    | [`Proxy`](./contracts/universal/Proxy.sol) | High-level interface for sending messages to and receiving messages from Optimism                   |
| [`L1StandardBridge`](../../specs/bridges.md)                                             | [`Proxy`](./contracts/universal/Proxy.sol)                              | Standardized system for transfering ERC20 tokens to/from Optimism                                   |
...
4. Run `npx hardhat forge-contract-verify --network <network-name>` to verify contracts on Etherscan.
   Make sure that the `ETHERSCAN_API_KEY` and `L1_RPC` variables are set.
