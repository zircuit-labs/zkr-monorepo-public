#!/usr/bin/env bash
# upgrade the L2OutputOracle and Verifier contract
...
forge script -vvv scripts/upgrades/104-L2OOVerifier.s.sol:L2OOVerifierDeploy --rpc-url "${UPGRADE_RPC}" --broadcast --mnemonics "${MNEMONIC_DEPLOYER}" --sender "${SENDER}"

echo "> Generating hardhat artifacts"
forge script -vvv scripts/upgrades/104-L2OOVerifier.s.sol:L2OOVerifierDeploy --sig 'sync()' --rpc-url "${UPGRADE_RPC}" --sender "${SENDER}"
