#!/usr/bin/env bash
# upgrade OptimismPortal contract
...
forge script -vvv scripts/upgrades/107-OptimismPortal.s.sol:OptimismPortalDeploy --rpc-url "${UPGRADE_RPC}" --broadcast --mnemonics "${MNEMONIC_DEPLOYER}" --sender "${SENDER}" --verify

echo "> Generating hardhat artifacts"
forge script -vvv scripts/upgrades/107-OptimismPortal.s.sol:OptimismPortalDeploy --sig 'sync()' --rpc-url "${UPGRADE_RPC}" --sender "${SENDER}"
