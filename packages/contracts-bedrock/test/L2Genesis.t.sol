...
contract L2GenesisTest is Test {
    ...
    function _test_genesis_predeploys(string memory _path) internal {
        ...
        // 15 proxies have the implementation set
        // can be verified by looking at output of test and checking if any predeploy
        // is missing in libraries/Predeploys.sol
        assertEq(getPredeployCountWithSlotSet(_path, Constants.PROXY_IMPLEMENTATION_ADDRESS), 15);
    ...
    function _test_allocs_size(string memory _path) internal {
        ...
        expected += 16; // predeploy implementations as counted in setPredeployImplementations
        expected += 256; // precompiles
        expected += 12; // preinstalls
        expected += 1; // 4788 deployer account
        // prefunded dev accounts are disabled for this test
