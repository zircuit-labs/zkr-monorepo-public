...
contract L1BlockEcotone_Test is L1BlockTest {
    ...
    /// @dev Tests that setL1BlockValuesEcotoneExclusions updates the values appropriately.
    function testFuzz_setL1BlockValuesEcotoneExclusions_succeeds(
        uint32 baseFeeScalar,
        uint32 blobBaseFeeScalar,
        uint64 sequenceNumber,
        uint64 timestamp,
        ...
        bytes32 hash,
        bytes32 batcherHash,
        uint8 depositExclusionsLength,
        bytes32 depositExclusionsBase
    )
        external
    {
        bytes memory depositExclusions = new bytes(depositExclusionsLength);
        for (uint256 i = 0; i < depositExclusionsLength; i++) {
            depositExclusions[i] = depositExclusionsBase[i % 32];
        }
        bytes memory functionCallDataPacked = Encoding.encodeSetL1BlockValuesEcotoneExclusions(
            baseFeeScalar, blobBaseFeeScalar, sequenceNumber, timestamp, number, baseFee, blobBaseFee, hash, batcherHash, depositExclusions
        );
        ...
        assertEq(l1Block.hash(), hash);
        assertEq(l1Block.batcherHash(), batcherHash);
        assertEq(l1Block.depositExclusions(), depositExclusions);
