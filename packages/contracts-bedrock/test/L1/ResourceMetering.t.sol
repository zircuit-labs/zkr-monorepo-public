...
contract ResourceMetering_Test is Test {
    ...
    function test_meter_initialResourceParams_succeeds() external view {
        (uint128 prevBaseFee, uint64 prevBoughtGas, uint64 prevBlockNum, uint64 prevTxCount) = meter.params();
    ...
    function test_meter_reinitializedResourceParams_succeeds() external {
        ...
        vm.store(address(meter), ForgeArtifacts.getInitializedSlot(), bytes32(uint256(0)));
        meter.initialize();

        (uint128 postBaseFee, uint64 postBoughtGas, uint64 postBlockNum, uint64 postTxCount) = meter.params();
        assertEq(prevBaseFee, postBaseFee);
        assertEq(prevBoughtGas, postBoughtGas);
        assertEq(prevBlockNum, postBlockNum);
        assertEq(prevTxCount, postTxCount);
    }
    ...
    function test_meter_updateParamsNoChange_succeeds() external {
        ...
        assertEq(postTxCount, prevTxCount + 1);
    }
    ...
    function test_meter_useMaxDeposits_succeeds() external {
        ResourceMetering.ResourceConfig memory rcfg = meter.resourceConfig();
        (,,, uint16 prevTxCount) = meter.params();
        for (uint256 i = 0; i < rcfg.maxTransactionLimit; i++) {
            meter.use(0);
        }
        (,,, uint16 postTxCount) = meter.params();
        assertEq(prevTxCount + rcfg.maxTransactionLimit, postTxCount);
    }

    function test_meter_exceedMaxDeposits_reverts() external {
        ResourceMetering.ResourceConfig memory rcfg = meter.resourceConfig();
        (,,, uint16 prevTxCount) = meter.params();
        for (uint256 i = 0; i < rcfg.maxTransactionLimit; i++) {
            meter.use(0);
        ...
        vm.expectRevert("ResourceMetering: too many deposits in this block");
        meter.use(0);
    }
}
