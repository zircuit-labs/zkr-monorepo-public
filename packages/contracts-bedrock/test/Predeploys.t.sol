...
contract PredeploysTest is CommonTest {
    ...
    function _isPredeploy(address _addr) internal pure returns (bool) {
        return _addr == Predeploys.L2_TO_L1_MESSAGE_PASSER || _addr == Predeploys.L2_CROSS_DOMAIN_MESSENGER
            ...
            || _addr == Predeploys.GAS_PRICE_ORACLE || _addr == Predeploys.WETH9
    ...
    function test_predeploys_succeeds() external view {
        ...
        for (uint256 i = 0; i < count; i++) {
            ...
            // OP tests can't seem to handle not having a governance token very well
            // se we just skip it explicitly here
            if (addr == Predeploys.GOVERNANCE_TOKEN) {
                continue;
            }
