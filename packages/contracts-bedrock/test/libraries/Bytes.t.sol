...
contract Bytes_slice_TestFail is Test {
    ...
    function testFuzz_slice_rangeOverflows_reverts(bytes memory _input, uint256 _start, uint256 _length) public {
        ...
        // this used to be done with assume, however it happened too frequently
        // that assume would reject too many inputs, leading to test failures
        // old check:
        // vm.assume(_start > type(uint256).max - _length);
        // we now use bound to force the value into the correct range instead
        vm.assume(_length > 0);
