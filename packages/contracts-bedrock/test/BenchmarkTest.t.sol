...
function setPrevBaseFee(Vm _vm, address _op, uint128 _prevBaseFee) {
    uint256 prevBaseFeeSlot = 0;
    _vm.store(address(_op), bytes32(prevBaseFeeSlot), bytes32((block.number << 192) | _prevBaseFee));
}
...
contract GasBenchMark_L1StandardBridge_Deposit is Bridge_Initializer {
    ...
    function test_bridgeETH_benchmark_0() external {
        ...
        setPrevBaseFee(vm, address(optimismPortal), 1 gwei);
        vm.resumeGasMetering();
        l1StandardBridge.bridgeETH{ value: 500 }(50000, hex"");
    }
